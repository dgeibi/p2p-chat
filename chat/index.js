...
function handleFile(socket, message) {
  ...
  events.emit('file-process-start', id);

  const processing = (check, percent, speed) => {
    if (check !== checksum) return;
    events.emit('file-processing', id, percent, speed);
  };
  ...
  const done = (check) => {
    ...
    events.emit('file-process-done', id);
  };
  ...
  const close = (check) => {
    ...
    md5.file(filepath, false, (md5Err, realChecksum) => {
      ...
      if (md5Err || !fileAccepted[realChecksum] || realChecksum !== checksum) {
        events.emit('file-receive-fail', { tag, username, filename, id });
      }
      events.emit('file-receiced', { tag, username, filename, filepath, id });
...
function handleSocket(socket, opts = {}) {
  ...
  socket.once('message', (session) => {
    ...
    events.emit('login', tag, username);
      ...
      switch (type) {
        ...
        case 'file-accepted': {
          fileModule.send(
            ...
            (e, filename) => {
              if (e) {
                logger.err('file-send-fail', filename, e.message);
                events.emit('file-send-fail', tag, username, filename, checksum, e.message);
              } else {
                events.emit('file-sent', tag, username, filename, checksum);
              }
            }
          );
          break;
...
function peopleLogout(socket) {
  ...
  if (localTag === local.tag) {
    ...
    events.emit('logout', tag, username);
  }
}
